...
def sequence_aware_loss(
    model, x0: torch.Tensor, t: torch.LongTensor, e: torch.Tensor, b: torch.Tensor, num_consecutive_steps=2, lamda=1
):
...
def get_loss_value(model, x0: torch.Tensor, t: torch.LongTensor, e: torch.Tensor, b: torch.Tensor):
    coef = calculate_alpha(b)
    at_bar = extract_into_tensor(coef["alphas_cumprod"], t)
    xt = x0 * torch.sqrt(at_bar) + e * torch.sqrt(1.0 - at_bar)
    eps_prediction = model(xt, t.float())
    ...
    for i in range(step, 0, -1):  # LOOP: step-1 timesteps
        ...
        dic[i] = d_t_1 + dic[i + 1] * torch.sqrt(at_1) * (
            1 - extract_into_tensor(coef["alphas_cumprod_prev"], torch.clamp(t - 1.0, min=0))
        ) / (1 - at_bar_prev)
    for key in dic.keys():
